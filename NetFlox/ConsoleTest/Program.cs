...
namespace ConsoleTest
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //{
            //    var film = entities.Films.Find(4);
            //    if (film != null)
            //    {
            //        Console.WriteLine($"Suppression du film {film.Titre}");
            //        entities.Films.Remove(film);
            //        entities.SaveChanges();
            //    }
            //}
            using (var entities = new NetFloxEntities())
            {
                foreach (var film in entities.Films)
                {
                    Console.WriteLine($"{film.Id} - {film.Titre} ({film.Annee})");
                }
            }
            Console.ReadLine();
        }
